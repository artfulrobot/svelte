...
If you don't need a full-fledged app framework and instead want to build a simple frontend-only site/app, you can also use Svelte (without Kit) with Vite by running `npm init vite` and selecting the `svelte` option. This will initialize the project and start a local development server. From within your project directory,  In future you can start the development server from within that directory with `npx vite . preview`. With this, `npm run build` will generate HTML, JS and CSS files inside the `dist` directory.  
